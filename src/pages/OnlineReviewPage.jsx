...
const PageContainer = styled.div`
  ...
  margin: 0 auto;
`;
const PaidFeatureBox = styled.div`
  padding: 36px 120px;   /* GridContainer와 동일하게 맞춤 */
  font-size: 28px;
  font-weight: bold;
  color: #17171b;
  text-align: center;
  min-height: 400px; /* 필요 시 세로 공간 확보 */
  display: flex;
  align-items: center;
  justify-content: center;
`;
...
function OnlineReviewPage() {
  ...
  const { analysisData, loading, error } = useGetReviewAnalysis(selectedOption);
  ...
  const isPaidFeature = analysisData?.status === 404;
    ...
    <PageContainer>
      <Header>
        <TitleSection>
          <Title>
            <TitleLarge>
              {isPaidFeature ? "유료 기능" : analysisData.storeName}
            </TitleLarge>
            <TitleSmall>
              {isPaidFeature ? "입니다!" : "의 전체 리뷰 분석"}
            </TitleSmall>
          </Title>
          {!isPaidFeature && (
            <QWrapper>
      ...
      {isPaidFeature ? (
        ...
        <GridContainer>
          ...
          <StyledReviewSection
            ...
            className="section-3"
            showSmallQ={false}
            ratios={analysisData.percentage}
          >
            <PieChart
              positive={analysisData.percentage.goodPercentage}
              neutral={analysisData.percentage.middlePercentage}
              negative={analysisData.percentage.badPercentage}
            />
          </StyledReviewSection>
          ...
          <StyledReviewSection
            ...
            popupImage={Popup2}
          >
            <KeywordBox>{analysisData.analysisKeyword}</KeywordBox>
          </StyledReviewSection>
